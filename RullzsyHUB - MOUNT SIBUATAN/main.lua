...
local jsonFolder = mainFolder .. "/json_mount_sibuatan_patch_new_001"
...
-- Server URL and JSON checkpoint file list
local baseURL = "https://raw.githubusercontent.com/0x0x0x0xblaze/json/refs/heads/main/json_mount_sibuatan/"
...
-- Fungsi utama untuk aktifkan god mode
local function enableGodMode()
    ...
    task.spawn(function()
        while godModeEnabled do
            ...
            if humanoid then
                -- Selalu jaga darah full
                humanoid.Health = humanoid.MaxHealth

                -- Jika ada damage, langsung reset
                ...
                -- Cegah kematian total
                humanoid.Died:Connect(function()
                    if godModeEnabled then
                        task.wait(0.1)
                        player:LoadCharacter()
                        task.wait(1)
                        enableGodMode() -- aktif ulang setelah respawn
                    end
                end)

                -- Hapus semua fall damage listener
                for _, conn in pairs(getconnections(humanoid.StateChanged)) do
                    conn:Disable()
                end

                -- Jika humanoid punya "BreakJointsOnDeath", nonaktifkan
...
-- Fungsi untuk matikan god mode
local function disableGodMode()
    godModeEnabled = false
end

local LoopingToggle = AutoWalkTab:CreateToggle({
    Name = "God Mode (Wajib Aktifkan)",
    CurrentValue = false,
    Callback = function(Value)
        if Value then
            enableGodMode()
...
-------------------------------------------------------------
-- AUTO WALK CHECKPOINT TOGGLES (SPAWNPOINT → 42)
-------------------------------------------------------------
...
-- WATER
local WaterToggle = AutoWalkTab:CreateToggle({
    Name = "Auto Walk (WaterCP)",
    CurrentValue = false,
    Callback = function(Value)
        if Value then
            playSingleCheckpointFile("watercp.json", 16)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
